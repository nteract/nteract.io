...
## nteract is local-only

<SocketDiagram />

The nteract daemon (`runtimed`) exposes two surfaces, both pinned to your machine.

Control runs over a Unix socket — `chmod 0600`, owner-only. The app and any MCP agents (Claude Code, Codex, Warp) connect here to run code, sync notebook state, and coordinate. There's no TCP control plane, no auth token, no port to scan. If you can open the socket file, you're you.
